...
源码结构
├── src/
│   ├── config_shared.py              ← 全局配置（路径、Prompt、Stop Strings）
│   ├── loader.py                     ← 模型加载器
│   ├── backend/                      ← 推理服务
│   │   ├── main.py                   ← uvicorn 入口
│   │   ├── app.py                    ← FastAPI 路由
│   │   ├── config.py                 ← 服务端配置
│   │   ├── model_manager.py          ← 模型管理 + 流式生成
│   │   └── generator.py              ← 文本生成 + Prompt 构建
│   ├── core/
│   │   ├── data/
│   │   │   ├── dataset.py            ← SFTDataset
│   │   │   ├── preprocessing.py      ← 数据预处理 + 学科标注
│   │   │   └── split_dataset.py      ← 原始数据 → train/test 分割
│   │   ├── train/
│   │   │   ├── train.py              ← 训练入口（SFT / KD / 教师 SFT）
│   │   │   ├── trainer.py            ← KGTrainer
│   │   │   ├── config.py             ← 训练配置
│   │   │   └── losses.py             ← KL 散度函数
│   │   └── scripts/
│   │       ├── merge_checkpoints.py  ← LoRA checkpoint → 完整模型
│   │       └── ropd_distill.py       ← ROP-D 黑盒蒸馏流水线
│   └── frontend/                     ← React 前端
│
├── data/
│   ├── Original_Data/                ← 原始高考 JSON
│   ├── Train_Data/                   ← 生成的 train.jsonl / test.jsonl
│   └── Example/                      ← ROP-D 产出 + 缓存
│
├── models/                           ← 原始 Qwen 模型
├── outputs/
│   ├── checkpoints/{sft,distillation,teacher-sft}/
│   ├── merged/{sft,distillation,teacher-sft}/
│   ├── saves/                        ← LoRA 临时快照
│   └── runs/                         ← TensorBoard
│
└── docs/1.md                         ← 本文档
...
# Step 1: 运行 ROP-D 流水线
python core/scripts/ropd_distill.py --base-model outputs/merged/sft/merged-sft-epoch_3 --max 10  # 先测试
python core/scripts/ropd_distill.py --base-model outputs/merged/sft/merged-sft-epoch_3           # 全量
